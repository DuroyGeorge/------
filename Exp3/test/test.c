...
// new syscall 78 function: print "No 78 syscall has changed to hello" and
// return a+b
#define sys_No 169

unsigned long old_sys_call_func;
unsigned long p_sys_call_table =
    0xffff000008af069; // find in  /boot/System.map-'uname -r'
...
void modify_syscall(void) {
  ...
  sys_call_addr = (unsigned long *)(p_sys_call_table + sys_No * 8);
...
static void mymodule_exit(void) { restore_syscall(); }
